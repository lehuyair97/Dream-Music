...
public class getSongsbyIDUsersAndIDPlaylist extends AsyncTask<Void, Void, Void> {
    private String idUsers, idPlaylist;
    private CallBackListener.SongsCallBack callback;

    public getSongsbyIDUsersAndIDPlaylist(String IDUsers, String IdPlaylist, CallBackListener.SongsCallBack callback) {
    ...
    protected Void doInBackground(Void... voids) {
        ...
        DatabaseReference playlistRef = FirebaseDatabase.getInstance().getReference().child("users").child(idUsers)
                .child("playlists").child(idPlaylist).child("Songs");
                                    ...
                                    userRef.addListenerForSingleValueEvent(new ValueEventListener() {
                                        ...
                                        public void onDataChange(DataSnapshot dataSnapshot) {
                                            ...
                                            if (userIds.size() == totalUsers[0]) {
                                                ...
                                                callback.onCallbackSong(listPlaylist);
                                            }
                                        }
                                        @Override
